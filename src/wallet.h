...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, 	// ONLY_NONDENOMINATED and not 1000 TPC at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                      // UTXO's that are valid for staking
};

// Possible states for zTPC send
enum ZerocoinSpendStatus {
    ZTPC_SPEND_OKAY = 0,                            // No error
    ZTPC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTPC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTPC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTPC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTPC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTPC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTPC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTPC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTPC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTPC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTPC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTPC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTPC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTPC_SPENT_USED_ZTPC = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZTPCAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000 TPC output and keys which can be used for the Masternode
