...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTPC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zTPC spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zTPC spend with serial %s from tx %s is not in valid range\n",
...
void RecalculateZTPCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTPC supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zTPC with serial %s is already in the block %d\n",
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZTPCMinted();
        RecalculateZTPCSpent();
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTPCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TPC to zTPC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTPC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTPC serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> tpcInputs;
          std::vector<CTxIn> zTPCInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zTPCInputs.push_back(stakeIn);
              }else{
                  tpcInputs.push_back(stakeIn);
              }
          }
          const bool hasTPCInputs = !tpcInputs.empty();
          const bool hasZTPCInputs = !zTPCInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasTPCInputs)
                      ...
                      for (CTxIn tpcIn : tpcInputs){
                          if(tpcIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zTPC inputs.
              if(hasZTPCInputs){
                  for (CTxIn zTpcInput : zTPCInputs) {
                      CoinSpend spend = TxInToZerocoinSpend(zTpcInput);
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-ztpc");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTPC mints and %d zTPC spends\n", __func__, nMints, nSpends);
...
{
	return CScript() << ParseHex("04649f3d9f077c31ad890fdb02b8e7ea50f1aeb4eabb9c963065f981808d75c0e2ae96fdfc58d11dfc6ee87b2d858bd2f612c4eb4536835ed6cfee48189231e719") << OP_CHECKSIG;
}
