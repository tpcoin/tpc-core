...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "tpc.conf"));
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TPC and zTPC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16521, 16384));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:16521"));
    ...
    strUsage += HelpMessageOpt("-backupztpc=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTPC minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizetpcamount=<n>     " + strprintf(_("Keep N TPC anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16384, 11691));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The TPC Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZTPCBackups = GetBoolArg("-backupztpc", true);
        pwalletMain->setZTPCAutoBackups(fEnableZTPCBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeTPCAmount = GetArg("-anonymizetpcamount", 0);
...
//    if (nAnonymizeTPCAmount < 2) nAnonymizeTPCAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1TPC+1000 == (.1TPC+100)*10
       10TPC+10000 == (1TPC+1000)*10
    */
