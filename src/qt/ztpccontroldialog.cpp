...
#include "ztpccontroldialog.h"
#include "ui_ztpccontroldialog.h"
...
std::list<std::string> ZTPCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTPCControlDialog::listMints;

ZTPCControlDialog::ZTPCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTPCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTPCControlDialog::~ZTPCControlDialog()
{
    delete ui;
}

void ZTPCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTPCControlDialog::updateList()
{
...
void ZTPCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTPCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTPC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));
}

std::vector<CZerocoinMint> ZTPCControlDialog::GetSelectedMints()
{
...
void ZTPCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
